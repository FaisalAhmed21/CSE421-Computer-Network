...
while True:
    ...
    while connected:
        ...
        if message.lower() == 'disconnect':
            ...
            worked_hours = int(message)
            salary = 0
            if worked_hours <= 40:
                salary = salary + (200 * worked_hours)

            else:
                salary = salary + 8000 + (300 * (worked_hours - 40))

        full_msg = f"Dear Client, your salary is: {salary}"
        server_socket.send(full_msg.encode(FORMAT))

        print("Received:", worked_hours)
